...
public class MainController {
	...
	public String TestInsert() {
		Long empMaxID = this.employeeRepositoryRef.getMaxId();
		long id = empMaxID+1;
		EmployeeEO employeeEORef = new EmployeeEO();
		int random = new Random().nextInt(3);
		String name = NAMES[random];
		employeeEORef.setId(id);
		employeeEORef.setEmployeeName(name);
		employeeEORef.setEmployeeNo("E"+id);
	...
	public String showAllEmployees() {
		...
		String html = "";
		for (EmployeeEO employeeEO : employees) {
			html = html+employeeEO+"<br>";
		}
		return html;
	}
}
